...
func Join(info mappingTable.ClusterInfo) bool {
	...
	if info.PlatformName == "gke" {
		...
		*/
		...
		var projectId = clusterRegisters.Spec.Projectid
		var fProjectId = flag.String("projectId", projectId, "specify a project id to examine")
		flag.Parse()
		if *fProjectId == "" {
		...
		for clusterName := range kubeConfig.Clusters {
			// clusterName "gke_keti-container_us-central1-a_gke-master"
			if clusterName == clusterRegisters.Spec.Clustername {
				join_cluster_config, err = clientcmd.NewNonInteractiveClientConfig(*kubeConfig, clusterName, &clientcmd.ConfigOverrides{CurrentContext: clusterName}, nil).ClientConfig()
				if err != nil {
					log.Println(err)
				}
...
/*
kubectl delete ns "kube-federation-system" --context gke_keti-container_us-central1-a_gke-master;
kubectl delete clusterroles.rbac.authorization.k8s.io kubefed-controller-manager:gke-master --context gke_keti-container_us-central1-a_gke-master ;
kubectl delete clusterrolebindings.rbac.authorization.k8s.io kubefed-controller-manager:gke-master-hcp --context gke_keti-container_us-central1-a_gke-master;
kubectl delete kubefedclusters gke-master -n kube-federation-system --context master
